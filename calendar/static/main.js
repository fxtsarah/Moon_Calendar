...
function calendarPage(month, year, day) {
    ...
    $.ajax({
        url: `/api/day_events/${month}/${day}/${year}`
    }).done(function(data) {
    ...
    for (let i = 1; i < lastDayInMonth + 1; i += 1){
        ...
        //console.log(moon_img);
        if(i == day){
             $('#main_container').append(`<a class="selectedday" id = "${i}" img src= "${moon_img}" onclick ='calendarPage(${month}, ${year}, ${i})')>${i}</a>`);
        } else {
        $('#main_container').append(`<a class="day" id = "${i}" img src= "${moon_img}" onclick ='calendarPage(${month}, ${year}, ${i})'>${i}</a>`);
        }
    }
